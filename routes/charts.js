...
const PREMIUM_ONLY_TYPES = ["transit", "solar_return", "lunar_return", "progressed"];
...
export function saveChart(user, body) {
  const { clientId, positions, houseCusps, aspects, ascLon, mcLon, housesReliable, engine, type,
    ...
    lunarReturnMoment, lunarReturnPlace, progressedTargetDate, progressedMoment, houseSystem } = body;
  ...
  if (PREMIUM_ONLY_TYPES.includes(type) && user.plan === "gratis") {
    const label = { transit: "Tránsitos", solar_return: "Revolución solar", lunar_return: "Retorno lunar", progressed: "Carta progresada" }[type];
  ...
  const id = newId("ch");
  db.prepare(`INSERT INTO charts (id, user_id, client_id, type, positions_json, house_cusps_json, aspects_json, asc_lon, mc_lon, houses_reliable, engine, solar_return_year, solar_return_moment, solar_return_place, transit_date, transit_natal_chart_id, lunar_return_moment, lunar_return_place, progressed_target_date, progressed_moment, house_system)
              VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(
    ...
    progressedTargetDate ?? null, progressedMoment ?? null,
    houseSystem || "equal"
  );
